...
gauge; and that the resulting Peierls bracket is well-defined on the
sufficiently local boundary class. These are the healthiness properties encoded
abstractly by \textbf{(P2)}--\textbf{(P4)}.
\end{remark}
...
the cosmology theory Peierls package (P1--P5), recorded here in abstract
interface form.
\end{theorem}
...
Assume $\mathbf{2T}_{\mathrm{Peierls}}$-\textbf{(P2)} so that the gauge-fixed
linearized renormalized IBVP $(P_{\mathrm{gf,ren}},B_\Lambda)$ is
Green--hyperbolic with unique advanced/retarded Green operators $E^\pm$ on the
chosen test-source space. Let $\Gamma_*$ be an involution on test sources and
linearized fields (e.g.\ pointer-basis transpose/real structure) such that
\[
P_{\mathrm{gf,ren}}\circ\Gamma_*=\Gamma_*\circ P_{\mathrm{gf,ren}},
\qquad
B_\Lambda\circ\Gamma_*=\Gamma_*\circ B_\Lambda.
\]
Then
\[
...
Assume $\mathbf{2T}_{\mathrm{Peierls}}$ and the hypotheses of
Lemma~\ref{lem:GammaStar-commutes-Greens}. Assume moreover that the renormalized
pairing used in the Peierls formula is $\Gamma_*$--invariant on first
functional derivatives (equivalently: the quadratic renormalized gauge-fixed
action pairing is $\Gamma_*$--invariant). Let $\Gamma$ act on functionals by
pullback along the induced swap on boundary data. Then
\[
...
Using $\Gamma_*$--invariance of the pairing gives the claim.
\end{proof}

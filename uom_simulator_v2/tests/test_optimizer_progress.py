...
class TestOptimizerProgress(unittest.TestCase):
    ...
    def test_progress_when_cap_inactive_and_theta_decreasing(self):
        ...
        def fake_by(params_batch, kernel_cfg, geom_consts, *args, **kwargs):
            logL = params_batch[:, 3]
            L = torch.exp(logL)
            C = 1.0
            p = 2.0
        ...
        def fake_conf(deltaC_batch, params_batch, kernel_cfg, geom_consts, *args, **kwargs):
        ...
        def fake_wall(params_batch, *args, **kwargs):
            B = params_batch.shape[0]
            return torch.zeros(B, dtype=torch.float64, device=params_batch.device)

        # Install fakes
        orig_by = p3.compute_by_batch
        orig_conf = p3.compute_conf_batch
        orig_wall = p3.compute_wall_penalty_batch
        ...
        try:
            res = p3.optimize_tick_p3(
                F_dS_k, F_dS_band_fraction, L_k, beta_k, self.cfg,
                d_sep=0.5, wall_work_bank=1.0, debug_print=True
            )
        finally:
            p3.compute_by_batch = orig_by
            p3.compute_conf_batch = orig_conf
            p3.compute_wall_penalty_batch = orig_wall

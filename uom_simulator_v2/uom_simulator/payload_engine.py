...
def knee_objective_log(ell: float, Lambda_rec: float, d_sep: float,  # d_sep is now a direct arg
    ...
    # Map windows: use a constant cL=1 so ℓ_max ≈ L4 * Λ (paper: linear in cutoff).
    W1 = window_ell(np.array([ell]), receiver_params.L4_init, Lambda_rec, 1.0,
                    receiver_params.deltaL_soft)[0]
    # Source window is disabled for retarded knee (receiver-side bandedge only).
    W2 = 1.0
...
def compute_retarded_knee(
    ...
    # deltaL_taper is now a relative width in ell-space: Δℓ ≈ deltaL_taper * ℓ*
    taper_width = max(1e-6, receiver_params.deltaL_taper * max(ell_star, 1.0))
    kw_taper = 1.0 / (1.0 + np.exp((ells - ell_star) / taper_width))

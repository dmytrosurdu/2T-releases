...
def window_ell(ell: np.ndarray, L4: float, Lambda: float, cL: float, deltaL: float) -> np.ndarray:
    ...
    The softening width is specified as a *relative* fraction of ℓ_max, so
    deltaL is interpreted as a ratio (e.g. 0.1 => 10% of ℓ_max).
    """
    ell_max = cL * L4 * Lambda
    deltaL_abs = max(deltaL * max(ell_max, 1e-12), 1e-9)
    return 1.0 / (1.0 + np.exp((ell - ell_max) / deltaL_abs))
